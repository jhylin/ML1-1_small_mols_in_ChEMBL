...
# Exclude unassigned mols only instead
df_sm = df.filter((pl.col("Type") == "Small molecule") & (pl.col("Structure Type") != "NONE") & (pl.col("Inorganic Flag") != 0 ))

df_sm #1,901,949 entries
```
...
+-------------------+--------------------------------------+--------------+-------------------+
| 22                | - Small molecules only               | 103.7 MB     | 1,901,949         |
|                   | - Exclude structure type with "NONE" |              |                   |
|                   | - Remove unassigned compounds        |              |                   |
+-------------------+--------------------------------------+--------------+-------------------+
