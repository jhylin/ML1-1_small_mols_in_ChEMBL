...
I have tried two different ways where one is using the write_parquet() by only adding file compression level (the "without partition" way), and the other one using use_pyarrow & pyarrow_options to partition datasets. The changes in parquet file size are shown in the following two tables.
...
|                   |                      | - two files produced for each max phase     |                   |
+-------------------+----------------------+---------------------------------------------+-------------------+
...
I'm glad it does work. The next series of posts will be about trying to use Polars dataframe library all the way with *Scikit_learn* (which I'm unsure if it is possible at the moment but I'll give it a try).