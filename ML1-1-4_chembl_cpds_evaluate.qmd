...
## getting a warning message after accuracy score generated - UserWarning: X does not have valid feature names, but LogisticRegression was fitted with feature names (?due to LR being previously fitted on the data already...)

## May need to save X_test & y_test as stand-alone datasets

y_mp = LR.predict(X_test)
```

note: 
* y_mp = predicted y (y_mp = LR.predict(X_test))  
