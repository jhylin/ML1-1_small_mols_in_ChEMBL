...
The following is basically a few different ways to remove the appropriate amount of null values from the original dataset.

```{python}
## Drop all rows with null entries
# df_dn = df_pa.drop_nulls()
# df_dn 
...
df_dn = df_pa.drop_nulls(subset="CX LogP")
df_dn
...
Both columns for "CX LogP" and "CX LogD" have null count of 28,141 for each, and the null count for each of the other 3 columns ("QED Weighted", "#RO5 Violations" and "Heavy atoms") is 27,922. I've chosen to use CX LogP column since it has a higher number of null counts, and later have found that these null counts do overlap within these 5 columns so after dropping all nulls in "CX LogP" column, these 5 columns should all have zero null counts.
